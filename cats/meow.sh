...
EOF
    ...
    ginkgo -slowSpecThreshold=1200 -flakeAttempts=3 ${@:2} .
  popd > /dev/null
}
...
EOF
  ...
  pushd "$test_path" >/dev/null
    ...
    ginkgo -r -race -slowSpecThreshold=1200 -flakeAttempts=3 ${@:2} smoke_tests http_routes tcp_routing
  popd >/dev/null
}
...
EOF
  ...
  pushd "$persi_test_dir" >/dev/null
    ...
    ginkgo -r -slowSpecThreshold=1200 -flakeAttempts=3 ${@:2} .
  popd >/dev/null
}
